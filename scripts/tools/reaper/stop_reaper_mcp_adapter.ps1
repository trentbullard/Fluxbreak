# Stops the REAPER MCP adapter if it is running.
...
if (-not $adapterProcessId) {
    ...
    if ($listeningProcessId -and (Test-IsAdapterProcess -ProcessId $listeningProcessId)) {
        ...
        Write-Error "Port $AdapterPort is in use by process $listeningProcessId, and it is not the REAPER MCP adapter."
        exit 1
    }
}
...
if (-not $adapterProcessId) {
    ...
    Write-Output "REAPER MCP adapter is not running."
    exit 0
}
...
Remove-Item $pidFilePath -ErrorAction SilentlyContinue
Write-Output "Stopped REAPER MCP adapter (PID $adapterProcessId)."
